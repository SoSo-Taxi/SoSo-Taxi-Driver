...
public class DrivingRouteOverlay extends OverlayManager {
    ...
    @Override
    public final List<OverlayOptions> getOverlayOptions(boolean isMarkerChanged) {
        if (mRouteLine == null) {
            return null;
        }
        ...
        if (mRouteLine.getAllStep() != null
            ...
            for (int j=0;j<steps.size();j=j+4) {
                DrivingStep step=steps.get(j);
                Bundle b = new Bundle();
                b.putInt("index", j);
                ...
                // 最后路段绘制出口点
                if (j == steps.size()-1 && step.getExit() != null) {
                    overlayOptions.add((new MarkerOptions())
        ...
        if (mRouteLine.getStarting() != null) {
            if(isMarkerChanged){
                overlayOptions.add((new MarkerOptions())
                        ...
                        .icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_current_marker)).zIndex(10));
            }else{
                overlayOptions.add((new MarkerOptions())
                        .position(mRouteLine.getStarting().getLocation())
                        .icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_start_marker)).zIndex(10));
            }
