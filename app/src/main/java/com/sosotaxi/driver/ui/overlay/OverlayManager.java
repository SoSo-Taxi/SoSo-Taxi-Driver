 ...
 * <p>
 * 复写{@link #getOverlayOptions(boolean isMarkerChanged)} 设置欲显示和管理的Overlay列表
 * </p>
 * <p>
 * 通过
...
public abstract class OverlayManager implements OnMarkerClickListener, OnPolylineClickListener {
     ...
     * @return 管理的Overlay列表
     */
    public abstract List<OverlayOptions> getOverlayOptions(boolean isMarkerChanged);

    /**
     * 将所有Overlay 添加到地图上
     */
    public final void addToMap(boolean isMarkerChanged) {
        if (mBaiduMap == null) {
            return;
        }

        removeFromMap();
        List<OverlayOptions> overlayOptions = getOverlayOptions(isMarkerChanged);
        if (overlayOptions != null) {
            mOverlayOptionList.addAll(getOverlayOptions(isMarkerChanged));
        }
